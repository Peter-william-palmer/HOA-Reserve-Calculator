...
with tab1:
    ...
    fig_gauge = go.Figure(go.Indicator(
        ...
        domain = {'x': [0, 1], 'y': [0, 1]},
        title = {'text': "Current Percent Funded"},
        gauge = {
            'axis': {'range': [None, 130]},
            'bar': {'color': "black"},
            'steps': [
    ...
    # 2. CASH FLOW CHART
    st.subheader("30-Year Cash Flow Analysis")
    ...
    fig_bar = px.bar(
        ...
        title="Projected Year-End Cash Balance"
    )
    ...
    # 3. PERCENT FUNDED CHART (NEW!)
    st.subheader("30-Year Percent Funded Trajectory")
    
    fig_pct = px.line(
        proj_df,
        x='Year',
        y='Percent Funded',
        title="Projected Percent Funded Status Over Time",
        labels={'Percent Funded': 'Percent Funded (%)'}
    )
    # Add Zone Lines
    fig_pct.add_hline(y=70, line_dash="dash", line_color="green", annotation_text="Healthy Target (70%)")
    fig_pct.add_hline(y=30, line_dash="dash", line_color="red", annotation_text="Critical (30%)")
    
    # Fill area below line
    fig_pct.update_traces(fill='tozeroy', line=dict(color='blue', width=3))
    fig_pct.update_layout(yaxis_range=[0, max(110, proj_df['Percent Funded'].max())])
    ...
    with col_solver:
        ...
        if st.button("Calculate Minimum Contribution"):
            ...
            for i in range(1000): 
                ...
                # Check year 5 percent funded
                yr5_pct = temp_proj.iloc[4]['Percent Funded']
